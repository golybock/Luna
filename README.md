...
## Оглавление
- [Производительность и High-Load](#-производительность-и-high-load)
...
## Производительность и High-Load

Платформа оптимизирована под высокие нагрузки (результаты замеров на `AMD Ryzen 5 5600, 12 потоков`, `WSL2 Docker`):

* **Авторизованный шлюз (Gateway API):** **7 515 RPS** при полной криптографической проверке JWT-подписей и lookup-запросе в Redis (средняя задержка **3.0 мс**).
* **Сквозной HTTP-сценарий (SignIn ➔ Workspace ➔ Pages):** **1 823 RPS** при 15 параллельных VU (создание страницы — **9.7 мс**, чтение — **4.8 мс**).
* **WebSocket SignalR (realtime-редактирование):** Стабильная обработка непрерывных циклов чтения-записи на уровне **76 RPS** с задержкой ~190 мс на чтение всей структуры TipTap-страницы с блоками из MongoDB.

#### Схема архитектуры нагрузочного тестирования и пропускной способности:
![Performance Schema](assets/perf_schema.png)

**Все изменения по оптимизации находятся в ветке `high-load-optimization`**, 
**оригинальная ветка `master` не была затронута после оптимизации.** 
...
![select workspace](assets/search_workspace.png)
